...
Stack conventions are **path-scoped rules** that auto-load when the matching files are open: `~/.cursor/rules/aws-cdk.md`. Applies to the main agent too, not just this subagent — don't restate it here.

## Memory

Persistent project memory lives at `.cursor/agent-memory/devops-engineer/` (committed alongside the project, shareable with the team). `MEMORY.md` is loaded into your system prompt automatically.
