...
- **Convention drift detection** — does this branch respect `.cursor/rules/` / `AGENTS.md or CLAUDE.md` / project style configs?
...
- **Apply project conventions, don't restate them.** If `.cursor/rules/`, `AGENTS.md or CLAUDE.md`, `.editorconfig`, or style configs exist, *use* them as review criteria; don't echo them back into the report.
...
Persistent project memory lives at `.cursor/agent-memory/code-reviewer/` (committed alongside the project, shareable with the team). `MEMORY.md` is loaded into your system prompt automatically.
...
- **Project conventions to apply** — non-obvious patterns from `.cursor/rules/`, `AGENTS.md or CLAUDE.md`, etc., distilled into review criteria
